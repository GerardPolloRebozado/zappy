...
use crate::common::utils::direction::calc_k;
...
pub enum ServerEvent {
    ...
    Message { player_id: u32, message: String, x: u32, y: u32 },
    ...
    NewPlayer { player_id: u32, x: u32, y: u32, orientation: u8, level: u8, team: String },
    /// Incantation started: GUI `pic X Y L #n ...`.
    StartIncantation { x: u32, y: u32, level: u8, player_ids: Vec<u32> },
    ...
    EggLaid { egg_id: u32, player_id: u32, x: u32, y: u32 },
...
impl ServerEvent {
    // Builder functions
    pub fn message(broadcaster: &Player, message: impl Into<String>) -> Self { Self::Message { player_id: broadcaster.id, message: message.into(), x: broadcaster.x, y: broadcaster.y } }
    pub fn eject(victim: &Player, source: &Player) -> Self { Self::Eject { player_id: victim.id, x: source.x, y: source.y } }
    pub fn death(player: &Player) -> Self { Self::Dead { player_id: player.id } }
    pub fn new_player(player: &Player, level: u8, team: impl Into<String>) -> Self { Self::NewPlayer { player_id: player.id, x: player.x, y: player.y, orientation: player.orientation, level, team: team.into() } }
    pub fn start_incantation(x: u32, y: u32, level: u8, player_ids: Vec<u32>) -> Self { Self::StartIncantation { x, y, level, player_ids } }
    pub fn egg_lay(player: &Player) -> Self { Self::EggLay { player_id: player.id } }
    pub fn resource_drop(player: &Player, resource: u8) -> Self { Self::ResourceDrop { player_id: player.id, resource } }
    pub fn resource_collect(player: &Player, resource: u8) -> Self { Self::ResourceCollect { player_id: player.id, resource } }
    pub fn egg_laid(egg_id: u32, player: &Player) -> Self { Self::EggLaid { egg_id, player_id: player.id, x: player.x, y: player.y } }
    ...
    pub fn to_gui_string(&self) -> Option<String> {
        match self {
            ServerEvent::Message { player_id, message, .. } => Some(format!("pbc #{player_id} {message}\n")),
            ServerEvent::Eject { player_id, .. } => Some(format!("pex #{player_id}\n")),
            ServerEvent::Dead { player_id } => Some(format!("pdi #{player_id}\n")),
            ServerEvent::NewPlayer { player_id, x, y, orientation, level, team } => {
                Some(format!("pnw #{player_id} {x} {y} {orientation} {level} {team}\n"))
            }
            ServerEvent::StartIncantation { x, y, level, player_ids } => {
                let players = player_ids.iter().map(|id| format!("#{id}")).collect::<Vec<_>>().join(" ");
            ...
            ServerEvent::ResourceDrop { player_id, resource } => Some(format!("pdr #{player_id} {resource}\n")),
            ServerEvent::ResourceCollect { player_id, resource } => Some(format!("pgt #{player_id} {resource}\n")),
            ServerEvent::EggLaid { egg_id, player_id, x, y } => Some(format!("enw #{egg_id} #{player_id} {x} {y}\n")),
    ...
    ///
    /// ```
    /// use zappy_server::common::{Player, protocol::ServerEvent};
    ///
    ...
    /// ```
    pub fn to_ai_string(&self, for_player: Option<&Player>, map_width: u32, map_height: u32) -> Option<String> {
        match self {
...
mod tests {
    ...
    fn test_dead_formats() {
        ...
        assert_eq!(event.to_ai_string(Some(&for_player), 10, 10), Some("dead\n".to_string()));
    }

    #[test]
    fn test_eject_and_broadcast_gui() {
        let eject = ServerEvent::Eject { player_id: 3, x: 1, y: 2 };
        assert_eq!(eject.to_gui_string(), Some("pex #3\n".to_string()));

        let message = ServerEvent::Message { player_id: 2, message: "hello".to_string(), x: 4, y: 5 };
        assert_eq!(message.to_gui_string(), Some("pbc #2 hello\n".to_string()));
    }

    #[test]
    fn test_eject_k_relative_to_orientation() {
        let event = ServerEvent::Eject { player_id: 1, x: 5, y: 4 };

        let facing_north = Player::new(1, 5, 5, 1);
        assert_eq!(event.to_ai_string(Some(&facing_north), 10, 10), Some("eject: 1\n".to_string()));

        let facing_east = Player::new(1, 5, 5, 2);
        assert_eq!(event.to_ai_string(Some(&facing_east), 10, 10), Some("eject: 3\n".to_string()));
    }
    ...
    fn test_constructors_use_player_fields() {
        ...
        assert!(matches!(event, ServerEvent::Message { player_id: 7, message, x: 2, y: 3, } if message == "hi"));
    }

    #[test]
    fn test_gui_event_formats() {
        assert_eq!(
            ServerEvent::NewPlayer { player_id: 1, x: 3, y: 4, orientation: 2, level: 5, team: "TeamA".to_string() }.to_gui_string(),
            Some("pnw #1 3 4 2 5 TeamA\n".to_string())
        );

        assert_eq!(
            ServerEvent::StartIncantation { x: 1, y: 2, level: 3, player_ids: vec![4, 5] }.to_gui_string(),
            Some("pic 1 2 3 #4 #5\n".to_string())
        );

        assert_eq!(
            ServerEvent::EndIncantation { x: 1, y: 2, result: 1 }.to_gui_string(),
            Some("pie 1 2 1\n".to_string())
        );

        assert_eq!(ServerEvent::EggLay { player_id: 2 }.to_gui_string(), Some("pfk #2\n".to_string()));

        assert_eq!(
            ServerEvent::ResourceDrop { player_id: 1, resource: 3 }.to_gui_string(),
            Some("pdr #1 3\n".to_string())
        );

        assert_eq!(
            ServerEvent::ResourceCollect { player_id: 1, resource: 0 }.to_gui_string(),
            Some("pgt #1 0\n".to_string())
        );

        assert_eq!(
            ServerEvent::EggLaid { egg_id: 10, player_id: 2, x: 5, y: 6 }.to_gui_string(),
            Some("enw #10 #2 5 6\n".to_string())
        );

        assert_eq!(ServerEvent::EggConnect { egg_id: 7 }.to_gui_string(), Some("ebo #7\n".to_string()));
        assert_eq!(ServerEvent::EggDeath { egg_id: 8 }.to_gui_string(), Some("edi #8\n".to_string()));

        assert_eq!(
            ServerEvent::EndOfGame { team: "TeamA".to_string() }.to_gui_string(),
            Some("seg TeamA\n".to_string())
        );

        assert_eq!(
            ServerEvent::ServerMessage { message: "server info".to_string() }.to_gui_string(),
