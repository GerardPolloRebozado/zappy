...
pub fn handle_request(server: &mut Server, client_uuid: &str, request: Request) {
    ...
    match request.command {
        ...
        Command::Msz => {
            let width = server.map.width;
            let height = server.map.height;
            let client = match server.clients.get_mut(client_uuid) {
                Some(c) => c,
                None => return,
            };
        ...
        Command::Bct(x, y) => {
            let data = match map::get_tile_content(&server.world, x, y) {
                Some(d) => d,
                None => return,
            };
            let client = match server.clients.get_mut(client_uuid) {
        ...
        Command::Mct => {
            ...
            for y in 0..height {
                for x in 0..width {
                    if let Some(data) = map::get_tile_content(&server.world, x, y) {
                        responses.push(data);
                    }
                }
            }
