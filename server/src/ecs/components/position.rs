...
impl Position {
    ...
    /// Steps one tile along the inhabitant's facing (`Forward` = north, `Right` = east,
    /// `Back` = south, `Left` = west). Other orientations leave the position unchanged.
...
mod tests {
    ...
    fn move_forward_east() {
        let mut pos = Position { x: 5, y: 5 };
        pos.move_forward(RelativeOrientation::Right, 10, 10);
    ...
    fn move_forward_south() {
        let mut pos = Position { x: 5, y: 5 };
        pos.move_forward(RelativeOrientation::Back, 10, 10);
    ...
    fn move_forward_changed_false_for_non_cardinal() {
        ...
        assert!(!pos.move_forward_changed(RelativeOrientation::BackLeft, 10, 10));
